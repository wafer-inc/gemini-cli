...
export interface CliArgs {
  ...
  localModelTimeout: number | undefined;
}
...
export async function parseArguments(): Promise<CliArgs> {
  const yargsInstance = yargs(hideBin(process.argv))
    ...
    .option('local-model-url', {
      type: 'string',
      description:
        'URL of local model server (e.g., http://localhost:8000). Use with GEMINI_AUTH_TYPE=local-model',
      default: process.env.GEMINI_LOCAL_MODEL_URL || 'http://localhost:8000',
    })
    .option('local-model-timeout', {
      type: 'number',
      description:
        'Timeout in milliseconds for local model requests',
      default: parseInt(process.env.GEMINI_LOCAL_MODEL_TIMEOUT || '30000'),
    })
...
export async function loadCliConfig(
  ...
  return new Config({
    ...
    ideMode,
    localModelUrl: argv.localModelUrl,
    localModelTimeout: argv.localModelTimeout,
  });
}
